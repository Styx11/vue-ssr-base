...
module.exports = (app, render) => {
  ...
  const temlpateWatcher = chokidar.watch(path.resolve(__dirname, '../src/template/index.html'));
  temlpateWatcher.on('change', filePath => {
    ...
    console.log(`template ${filePath} has changed, you need to refresh the browser`);
  });
  ...
  // 适配 Koa
  // 因为 hotMiddleware 内部会持续向客户端发送数据（event-stream），所以设 ctx.body = stream
